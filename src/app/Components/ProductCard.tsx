...
const ProductCard: React.FC<propsType> = ({ id, name, category, image, wish, fuel, mode, capacity, price }) => {
    return (
        <div className="max-w-[400px] p-5 bg-white rounded-lg items-center space-y-28">
            <div className="flex justify-between">
                <h1 className="text-[20px] text-left font-bold text-[#1A202C]">{name}<br /><span className="text-[14px] text-[#90A3BF]">{category}</span></h1>
                {/* <Image width={28} height={24} src={wish} alt={`logo-${id}`} /> */}
            </div>
            <Image className="w-auto h-auto" src={image} alt={name} />
            <div className="space-y-5">
                <div className="flex justify-between">
                    ...
                    <div className="flex gap-1">
                        ...
                        <p className="text-[16px] text-[#90A3BF] font-medium">{mode}</p>
                    </div>
                    <div className="flex gap-1">
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <div className="flex justify-between items-center">
                    <h2 className="text-[20px] text-[#1A202C] font-bold">${price}.00/<span className="text-[#90A3BF] text-[16px]">day</span></h2>
                    <button className="px-10 py-5 bg-[#3563E9] text-white font-semibold text-[16px] rounded-lg">Rent Now</button>
                </div>
            </div>
        </div>
    )
