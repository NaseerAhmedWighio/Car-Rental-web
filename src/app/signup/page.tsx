        ...
        <main className='w-screen h-screen bg-[#F6F7F9] overflow-hidden relative'>
            ...
            >
                <div className='relative w-[220px] sm:w-[300px] md:w-[400px]'>
                    <Image src={gtr} alt='Nissan GT-R' className='w-full h-auto drop-shadow-2xl' width={400} height={200} />
                </div>
            </motion.div>
            ...
            >
                ...
                <div className='hidden md:flex md:w-[45%] lg:w-[50%] flex-col justify-center px-8 lg:px-12 xl:px-20 space-y-6'>
                    ...
                    <h2 className='text-[#1A202C] text-[24px] lg:text-[30px] xl:text-[36px] font-bold leading-tight'>
                        Join the <span className='text-[#3563E9]'>fast lane</span>
                    </h2>
                    <p className='text-[#90A3BF] text-[14px] lg:text-[16px] leading-relaxed max-w-md'>
                        Create your account to unlock premium car rentals, exclusive deals, and a seamless booking experience.
                    </p>
                    <div className='flex items-center gap-3'>
                        <div className='w-10 h-10 bg-[#3563E9] rounded-full flex items-center justify-center'>
                            <svg width='20' height='20' viewBox='0 0 24 24' fill='none'><path d='M13 10V3L4 14h7v7l9-11h-7z' stroke='white' strokeWidth='2' strokeLinecap='round' strokeLinejoin='round'/></svg>
                        </div>
                        <span className='text-[#1A202C] text-[14px] lg:text-[15px] font-medium'>Quick signup in under 1 minute</span>
                    </div>
                </div>
                        ...
                        {showVerification ? (
                            <>
                                <div className='text-center'>
                                    <h3 className='text-[#1A202C] text-xl sm:text-2xl font-bold'>Verify Email</h3>
                                    <p className='text-[#90A3BF] text-xs sm:text-sm mt-1 truncate'>Code sent to {email}</p>
                                </div>
                                <form onSubmit={handleVerification} className='space-y-3'>
                                    <input type='text' value={verificationCode} onChange={e => setVerificationCode(e.target.value)} className='w-full px-3 py-2.5 border border-[#C3D4E9] rounded-xl outline-none focus:border-[#3563E9] text-sm text-center tracking-[0.3em]' placeholder='••••••' maxLength={6} required />
                            ...
                            <>
                                <div className='text-center'>
                                    <h3 className='text-[#1A202C] text-xl sm:text-2xl font-bold'>Create Account</h3>
                                    <p className='text-[#90A3BF] text-xs sm:text-sm mt-1'>Choose your preferred method</p>
                                </div>

                                {/* Social Buttons */}
                                <div className='space-y-2'>
                                    {socialProviders.slice(0, 2).map(provider => (
                                        <button key={provider.name} onClick={() => handleOAuthSignUp(provider.strategy)} className={`w-full flex items-center gap-3 px-4 py-2.5 border ${provider.border} ${provider.bg} rounded-xl transition-colors`}>
                                            <SocialIcon name={provider.icon} size={18} />
                                            <span className='text-[#1A202C] font-medium text-sm'>Sign up with {provider.name}</span>
                                        </button>
                                    ))}
                                </div>
                                ...
                                <div className='grid grid-cols-3 gap-2'>
                                    {socialProviders.slice(2).map(provider => (
                                        <button key={provider.name} onClick={() => handleOAuthSignUp(provider.strategy)} className={`flex items-center justify-center gap-1.5 px-3 py-2.5 border ${provider.border} ${provider.bg} rounded-xl transition-colors`}>
                                ...
                                <form onSubmit={handleEmailSignUp} className='space-y-3'>
                                    <div className='grid grid-cols-2 gap-2'>
                                        <input type='text' value={firstName} onChange={e => setFirstName(e.target.value)} className='w-full px-3 py-2.5 border border-[#C3D4E9] rounded-xl outline-none focus:border-[#3563E9] text-sm transition-colors' placeholder='First' required />
                                        <input type='text' value={lastName} onChange={e => setLastName(e.target.value)} className='w-full px-3 py-2.5 border border-[#C3D4E9] rounded-xl outline-none focus:border-[#3563E9] text-sm transition-colors' placeholder='Last' required />
                                    </div>
                                    ...
                                    <input type='password' value={password} onChange={e => setPassword(e.target.value)} className='w-full px-3 py-2.5 border border-[#C3D4E9] rounded-xl outline-none focus:border-[#3563E9] text-sm transition-colors' placeholder='Min. 8 characters' minLength={8} required />
                                    {error && <p className='text-red-500 text-xs'>{error}</p>}
                                    <button type='submit' disabled={loading} className='w-full bg-[#3563E9] hover:bg-[#2851c7] disabled:opacity-50 text-white font-bold py-2.5 rounded-xl transition-colors text-sm'>
                                ...
                                <div className='text-center text-[#90A3BF] text-xs'>
                                    Already have an account? <a href='/signIn' className='text-[#3563E9] font-semibold hover:underline'>Sign In</a>
                                </div>
                            </>
                        )}
                    </div>
